...
function Toolbar({ onNew, onImportFiles, onExportZip, baseUrl, setBaseUrl }: {
  onNew: () => void;
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div className="grid gap-2 w-full sm:max-w-lg">
        <Label htmlFor="baseUrl" className="text-xs">Base URL du site public</Label>
        <div className="flex gap-2">
          <Input id="baseUrl" placeholder="https://ton-domaine" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} />
          <Button variant="secondary" onClick={() => setBaseUrl("https://ton-domaine")}>Reset</Button>
        </div>
        <p className="text-xs text-gray-500">Le viewer vit à <code className="font-mono">/m/index.html</code> et lit <code className="font-mono">/data/&lt;id&gt;.json</code>.</p>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <Button variant="outline" onClick={() => fileRef.current?.click()}>Importer JSON…</Button>
        <Button variant="outline" onClick={onExportZip}>Exporter ZIP</Button>
      </div>
    </div>
  );
}
    ...
    <div className="rounded-2xl border bg-white">
      <Table>
        ...
        <TableBody>
          {cards.length === 0 && (
            <TableRow>
              <TableCell colSpan={6} className="text-center text-gray-500 py-10">Aucune fiche.</TableCell>
            </TableRow>
          )}
          {cards.map((c) => (
            <TableRow key={c.id}>
              ...
              <TableCell>{c.urgence?.nom ? (<span className="inline-flex items-center gap-2"><Badge>{c.urgence.nom}</Badge><span className="text-xs text-gray-500">{c.urgence.telephone}</span></span>) : <span className="text-gray-500">—</span>}</TableCell>
              <TableCell className="text-sm text-gray-500">{toDisplayDate(c.last_update)}</TableCell>
              <TableCell className="text-right">
                <div className="flex justify-end gap-2">
                  <Button size="icon" variant="outline" title="Éditer" onClick={() => onEdit(c.id)}><Edit className="h-4 w-4"/></Button>
                  <Button size="icon" variant="outline" title="Télécharger JSON" onClick={() => onDownload(c.id)}><Download className="h-4 w-4"/></Button>
                  <Button size="icon" variant="outline" title="QR / URL / NFC" onClick={() => onQr(c.id)}><QrCode className="h-4 w-4"/></Button>
                  <Button size="icon" variant="destructive" title="Supprimer" onClick={() => onDelete(c.id)}><Trash2 className="h-4 w-4"/></Button>
                </div>
              </TableCell>
            </TableRow>
          ))}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button type="button" variant="outline" onClick={() => set("token", nanoid(16))}>Générer</Button>
                  <Button type="button" variant="outline" onClick={() => set("token", "")}>Effacer</Button>
                </div>
              </div>
            </div>
          </TabsContent>
...
function QrDialog({ open, setOpen, card, baseUrl }: { open: boolean; setOpen: (v: boolean) => void; card: MedicalCard | null; baseUrl: string; }) {
  if (!card) return null;
  ...
  return (
    <Dialog open={open}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>Partager / Encoder</DialogTitle></DialogHeader>
        <div className="grid gap-4">
          <div className="flex items-center justify-center">
            <div className="p-4 rounded-xl border bg-white"><QRCodeCanvas value={url} size={192} includeMargin /></div>
          </div>
          <div className="grid gap-2">
...
function ConfirmDialog({ open, setOpen, onConfirm, title, description }: { open: boolean; setOpen: (v: boolean) => void; onConfirm: () => void; title: string; description: string; }) {
  return (
    <Dialog open={open}>
      <DialogContent>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-6xl p-5 md:p-8">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">Fiches <Badge>{cards.length}</Badge></CardTitle>
          </CardHeader>
          <CardContent>
