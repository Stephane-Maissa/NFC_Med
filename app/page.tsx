...
function cardToJson(card: MedicalCard) {
  return {
    ...
    ...(card.token ? { token: card.token } : {}), // uniquement si vous voulez exploiter un token côté viewer
  };
}
...
function fromJsonToCard(json: any, id?: string): MedicalCard {
  return {
    ...
    allergies: Array.isArray(json?.allergies) ? json.allergies : (json?.allergies ? String(json.allergies).split(/\s*,\s*|\n+/).filter(Boolean) : []),
...
function Toolbar({ onNew, onImport, onExportZip, baseUrl, setBaseUrl }: {
  onNew: () => void;
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div className="grid gap-2 w-full sm:max-w-lg">
        <Label htmlFor="baseUrl" className="text-xs uppercase tracking-wide text-muted-foreground">Base URL du site public</Label>
        ...
        <p className="text-xs text-muted-foreground">C'est la racine où vit votre viewer (/m/index.html). L'URL publique sera <code className="font-mono">{sanitizeBaseUrl(baseUrl) || "https://github.com/Stephane-Maissa"}/m/&lt;id&gt;</code>.</p>
      </div>

      <div className="flex gap-2">
        <Button onClick={onNew} className="gap-2"><Plus className="h-4 w-4"/>Nouvelle fiche</Button>
        <input ref={fileRef} type="file" accept="application/json" multiple className="hidden" onChange={(e) => onImport(e.target.files)} />
        <DropdownMenu>
    ...
    <div className="rounded-2xl border bg-card">
      <Table>
        ...
        <TableBody>
          {cards.length === 0 && (
            <TableRow>
              <TableCell colSpan={6} className="text-center text-muted-foreground py-10">Aucune fiche pour l'instant.</TableCell>
            </TableRow>
          )}
          {cards.map((c) => (
            <TableRow key={c.id}>
...
function EditDialog({ open, onOpenChange, card, onSave }: {
          ...
          <TabsContent value="identite" className="mt-4">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                ...
                <p className="text-xs text-muted-foreground">Si renseigné, l'URL inclura <code className="font-mono">?t=…</code> (à faire vérifier côté viewer si vous le souhaitez).</p>
              </div>
            </div>
          </TabsContent>
...
function QrDialog({ open, onOpenChange, card, baseUrl }: { open: boolean; onOpenChange: (v: boolean) => void; card: MedicalCard | null; baseUrl: string; }) {
  if (!card) return null;
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="grid gap-4">
          <div className="flex items-center justify-center">
            <div className="p-4 rounded-xl border bg-background"><QRCodeCanvas value={url} size={192} includeMargin /></div>
          </div>
          <div className="grid gap-2">
            ...
            <div className="flex gap-2">
              ...
              <Button asChild className="gap-2"><a href={url} target="_blank" rel="noreferrer"><Globe className="h-4 w-4"/>Ouvrir</a></Button>
            </div>
          </div>
          <div className="grid gap-2">
            ...
            <p className="text-xs text-muted-foreground">Nécessite votre script <code className="font-mono">tools/write_ntag215.py</code> et un lecteur compatible (ex. ACR122U).</p>
          </div>
          <div className="text-xs text-muted-foreground">Astuce: imprimez le QR et glissez-le dans le portefeuille de la personne, en plus de la puce.</div>
        </div>
      </DialogContent>
    </Dialog>
  );
...
function ConfirmDialog({ open, onOpenChange, onConfirm, title, description }: { open: boolean; onOpenChange: (v: boolean) => void; onConfirm: () => void; title: string; description: string; }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
...
export default function MedicalNFCAdmin() {
  ...
  const [baseUrl, setBaseUrl] = useState<string>(localStorage.getItem(BASEURL_KEY) || "https://github.com/Stephane-Maissa");
  ...
  async function exportZip() {
    ...
    const readme = `Déposez le contenu du dossier data/ à la racine de votre hébergement statique (\n` +
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-6xl p-5 md:p-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">Générez, éditez et exportez des fiches prêtes pour vos puces NTAG215 et votre site public.</p>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">Fiches <Badge variant="secondary">{cards.length}</Badge></CardTitle>
          </CardHeader>
          <CardContent>
            <CardsTable
              cards={cards}
        ...
        <ConfirmDialog open={!!deleteId} onOpenChange={(v) => !v && setDeleteId(null)} onConfirm={() => deleteCard(deleteId!)} title="Supprimer la fiche ?" description="Cette action est définitive (dans ce back‑office)." />

        <footer className="text-xs text-muted-foreground text-center mt-8">
          <div className="flex items-center justify-center gap-2">
            <ShieldCheck className="h-3.5 w-3.5"/>
            <span>Conseil confidentialité : protégez ce back‑office par mot de passe (Basic Auth / proxy) et utilisez des IDs opaques.</span>
          </div>
        </footer>
      </div>
    </div>
